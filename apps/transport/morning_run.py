"""A Driver's own morning pickup run: home to school, stop by stop."""
...
_RUN_STATUSES = {"notStarted", "inProgress", "arrivedSchool", "completed"}


class MorningRunHandler(EntityHandler):
    """One row per Driver per service day (entity id: membershipId:morning:serviceDate).

    Only the Driver it belongs to ever writes it, and only for today, on the route they
    are really, currently assigned to - see daily_run.py for what stays fixed once it
    exists. Transport Control reads every driver's run for its live operations overview.
    """

    entity_type = c.MORNING_RUN
    ...
    def clean(self, ctx: MutationContext) -> dict[str, Any]:
        ...
        if ctx.entity_id != f"{member_id}:morning:{service_date}":
            raise Rejected("The record id must match membershipId:morning:serviceDate.")
        ...
        return {
            ...
            "arrivedSchoolAt": text(p, "arrivedSchoolAt", max_len=40, required=False),
